...
// void Optimizer::OptimizeGraphGlobalMeas()
void Optimizer::OptimizeGraphGlobalMeas(Map* pMap)
{
    if(!pMap->HasKFRelatedToGlobalOrigin())    
        return;        
    ...
    /*typedef g2o::BlockSolver<g2o::BlockSolverTraits<3, 1>> BlockSolver_3_1; // TODO Is this OK???

    BlockSolver_3_1::LinearSolverType * linearSolver =
           new g2o::LinearSolverEigen<BlockSolver_3_1::PoseMatrixType>();
    ...
    vector<g2o::Sim3,Eigen::aligned_allocator<g2o::Sim3> > vScw(nMaxKFid+1);
    vector<g2o::Sim3,Eigen::aligned_allocator<g2o::Sim3> > vCorrectedSwc(nMaxKFid+1);
    //std::cout << "SIZE vector KFs: " << vpKFs.size() << std::endl;

    KeyFrame* kfRelatedToGpsOrigin = pMap->GetKFRelatedToGlobalOrigin(); 
    Sophus::SE3f Twb0 = kfRelatedToGpsOrigin->GetImuPose(); // TODO this pose should be optimized??? Here is used as a constant

    if(!kfRelatedToGpsOrigin->mGlobalMeasCalib.mbIsSet)
    {
        std::cerr << "Cannot find Global Sensor to body transform" << std::endl;
        throw std::invalid_argument("Missing Global Sensor to body transform!");
    }
    ...
    /*R << 0.33637906, -0.01747948,  0.94156444,
       -0.02077994, -0.99972206, -0.01113539,
        0.94149738, -0.01581994, -0.33664879;

    Eigen::Matrix<float, 3, 1> t;
    t << 0.06901, -0.02781, -0.12395;              */
    /*Sophus::SE3f Tbg = kfRelatedToGpsOrigin->mGlobalMeasCalib.mTbg;
    std::cout << "Tbg rotation\n" << Tbg.rotationMatrix() << std::endl;
    std::cout << "Tbg translation\n" << Tbg.translation() << std::endl;*/
    //Sophus::SE3f Tg0w = (Twb0 * Tbg).inverse();
    ...
    int edgesMeasCount = 0;
    KeyFrame* lastKF = static_cast<KeyFrame*>(NULL);
    // Set KeyFrame vertices
    for(size_t i=0, iend=vpKFs.size(); i<iend;i++)
    {        
        KeyFrame* pKFi = vpKFs[i];
        ...
        // TODO USAR TODOS LOS KEYFRAMES, NO SOLO LOS QUE TIENEN MEDICIONES DE GPS 
        // (quizás no usar los que están marcados como bad, ver en el resto de las optimizaciones como manejan eso)
            
        VertexPose4DoF* v = new VertexPose4DoF(pKFi); //VertexGlobalPosition * v = new VertexGlobalPosition();
        v->setId(pKFi->mnId);
        //v->setEstimate(Tg0gi.translation().cast<double>()); // set Estimate at the very beginning
        optimizer.addVertex(v);
        Sophus::SE3d Tcw_ = pKFi->GetPose().cast<double>();
        g2o::Sim3 Siw(Tcw_.unit_quaternion(),Tcw_.translation(),1.0);
        const int nIDi = pKFi->mnId;
        vScw[nIDi] = Siw;
        if(lastKF)
        {
            Sophus::SE3d Tciw = lastKF->GetPose().cast<double>();
            g2o::Sim3 Siw(Tciw.unit_quaternion(),Tciw.translation(),1.0);

            Sophus::SE3d Tcjw = pKFi->GetPose().cast<double>();
            g2o::Sim3 Sjw(Tcjw.unit_quaternion(),Tcjw.translation(),1.0);
            ...
            e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKFi->mnId)));
            e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(lastKF->mnId)));

            e->information() = matLambda;
            optimizer.addEdge(e);
        }
        
        if(pKFi->HasGlobalPositionMeas() && kfRelatedToGpsOrigin->HasGlobalPositionMeas())
        {
            throw std::logic_error("Not implemented yet");
            //std::cout << "Diff: " << std::abs(pKFi->mTimeStamp - pKFi->GetGlobalPositionMeas()->timestamp) << std::endl;
            //if(std::abs(pKFi->mTimeStamp - pKFi->GetGlobalPositionMeas()->timestamp) > 0.1)
            //    std::cout << "kf: " << pKFi->mTimeStamp << " meas: " <<  pKFi->GetGlobalPositionMeas()->timestamp << std::endl;
            /*Sophus::SE3f Twbi = pKFi->GetImuPose();
            Sophus::SE3f Twgi = Twbi * Tbg;
            Sophus::SE3f Tg0gi = Tg0w * Twgi;
            std::cout << "=======================" << std::endl;
            std::cout << "Estimate:" << std::endl;
            std::cout << Tg0gi.translation() << std::endl;                    */

            /*EdgeGlobalMeasurement* edge = new EdgeGlobalMeasurement(Twb0.cast<double>(), Tbg.cast<double>());
            edge->setVertex(0, v);
            
            Eigen::Vector3f relativeMeasurement = pKFi->GetGlobalPositionMeas()->getRelativeFromOrigin(*kfRelatedToGpsOrigin->GetGlobalPositionMeas());
                        
            edge->setMeasurement(relativeMeasurement.cast<double>());
            edge->setInformation(pKFi->GetGlobalPositionMeas()->getCovariance().inverse().cast<double>());
            optimizer.addEdge(edge);
            edgesMeasCount++;*/
        }
        lastKF = pKFi;
    }
    std::cout << "OptimizeGraphGlobalMeas(): Initialize Optimization! " 
            << edgesMeasCount << " edges of type EdgeGlobalMeasurement" << std::endl;
    optimizer.initializeOptimization();
    optimizer.optimize(13);    

    std::cout << "OptimizeGraphGlobalMeas(): DONE!" << std::endl;    
    
    for(size_t i=0, iend=vpKFs.size(); i<iend;i++)
    {
        KeyFrame* pKFi = vpKFs[i];
        if(pKFi->isBad())// || !pKFi->HasGlobalPositionMeas())
            continue;
        
        //std::cout << "MP loop: " << i << std::endl;
        VertexPose4DoF* Vi = static_cast<VertexPose4DoF*>(optimizer.vertex(pKFi->mnId));
...
void Optimizer::LocalInertialBAWithGlobalMeas(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges, bool alignmentRotationFixed, bool bLarge, bool bRecInit)
{
    Map* pCurrentMap = pKF->GetMap();
    ...
    KeyFrame* kfRelatedToGpsOrigin = pCurrentMap->GetKFRelatedToGlobalOrigin(); 
    int edgesMeasCount = 0;
    const unsigned long iniMPid = maxKFid*5;

    Eigen::Matrix3f Rgw = pCurrentMap->GetGlobalVIOAlignment();
    VertexRotation *VAlignment = new VertexRotation(Rgw);
    // The first (maxKFid + 1) * 4 identifiers are reserved 
    // for VertexPose, VertexVelocity, VertexGyroBias and VertexAccBias.
    // From iniMPid onwards, the identifiers are reserved for VertexSBAPointXYZ.
    // FIXME for a low number of keyframes, this id could cause problems
    int vertexAlignmentId = (maxKFid + 1) * 4 + 1;
    VAlignment->setId(vertexAlignmentId);
    VAlignment->setFixed(alignmentRotationFixed);
    optimizer.addVertex(VAlignment);

    // Set Local temporal KeyFrame vertices
    N=vpOptimizableKFs.size();
    int kfWithGlobalMeas = 0;
    int origEdges = 0;
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(kfRelatedToGpsOrigin && kfRelatedToGpsOrigin->HasGlobalPositionMeas() && kfRelatedToGpsOrigin->mGlobalMeasCalib.mbIsSet && pKFi->HasGlobalPositionMeas())
        {                      
            kfWithGlobalMeas++;
            for(const GlobalPosition::GlobalPosition* meas : pKFi->GetGlobalPositionMeas())
            {               
                const GlobalPosition::GlobalPosition* originMeas = kfRelatedToGpsOrigin->GetGlobalPositionMeas()[0];
                Sophus::SE3f Twb0 = kfRelatedToGpsOrigin->GetPoseInverse() * kfRelatedToGpsOrigin->GetPoseRelativeToKF(originMeas->id).inverse() * kfRelatedToGpsOrigin->mImuCalib.mTcb;
                Eigen::Vector3f tbg = kfRelatedToGpsOrigin->mGlobalMeasCalib.tbg;
                
                Sophus::SE3f Tcir = pKFi->GetPoseRelativeToKF(meas->id);                
                EdgeGlobalMeasurement* edge = new EdgeGlobalMeasurement(Twb0.cast<double>(), Tcir.cast<double>(), tbg.cast<double>());
                edge->setVertex(0, VP);
                edge->setVertex(1, VAlignment);

                Eigen::Vector3d relativeMeasurement = meas->getRelativeFromOrigin(*originMeas);
                double measurementNorm = relativeMeasurement.norm();
                if(measurementNorm > 1e-3)
                {
                    edge->setMeasurement(relativeMeasurement);
                    edge->setInformation(meas->getCovariance().inverse().cast<double>());//covariance.inverse());
                    optimizer.addEdge(edge);
                    edgesMeasCount++;
                    origEdges++;
                    break;
    ...
    /*std::cout << "Global Meas. Optimization: " 
            << edgesMeasCount << " edges of type EdgeGlobalMeasurement. opt_it: " << opt_it << std::endl;*/
    // Set Local visual KeyFrame vertices
    for(list<KeyFrame*>::iterator it=lpOptVisKFs.begin(), itEnd = lpOptVisKFs.end(); it!=itEnd; it++)
    {
        KeyFrame* pKFi = *it;
        ...
        VP->setFixed(false);
        optimizer.addVertex(VP);
        /*if(kfRelatedToGpsOrigin && kfRelatedToGpsOrigin->mGlobalMeasCalib.mbIsSet && pKFi->HasGlobalPositionMeas())
        {
            Sophus::SE3f Twb0 = kfRelatedToGpsOrigin->GetImuPose();
            Eigen::Vector3f tbg = kfRelatedToGpsOrigin->mGlobalMeasCalib.tbg;
            EdgeGlobalMeasurement* edge = new EdgeGlobalMeasurement(Twb0.cast<double>(), tbg.cast<double>());
            edge->setVertex(0, VP);
            edge->setVertex(1, VAlignment);
            
            Eigen::Vector3d relativeMeasurement = pKFi->GetGlobalPositionMeas()->getRelativeFromOrigin(*kfRelatedToGpsOrigin->GetGlobalPositionMeas());
            //std::cout << "Diff2: " << std::abs(pKFi->mTimeStamp - pKFi->GetGlobalPositionMeas()->timestamp) << std::endl;   
            edge->setMeasurement(relativeMeasurement);
            edge->setInformation(pKFi->GetGlobalPositionMeas()->getCovariance().inverse().cast<double>());
            optimizer.addEdge(edge);
            edgesMeasCount++;
    ...
    for(int i=0; i<N; i++)
    {
        ...
        VertexPose* VP = static_cast<VertexPose*>(optimizer.vertex(pKFi->mnId));
        // Exception thrown here sometimes (Rotation Matrix is not orthogonal)
        // Sophus::SE3f Tcw(VP->estimate().Rcw[0].cast<float>(), VP->estimate().tcw[0].cast<float>());
        Sophus::SE3f Tcw(Eigen::Quaternionf(VP->estimate().Rcw[0].cast<float>()), VP->estimate().tcw[0].cast<float>());
        pKFi->SetPose(Tcw);
        pKFi->mnBALocalForKF=0;
    ...
    pMap->IncreaseChangeIndex();
    VertexRotation* estimatedAlignment = static_cast<VertexRotation*>(optimizer.vertex(vertexAlignmentId));
    /*std::cout << "END ROTATION (WITH GLOBAL MEAS)"<< std::endl;
    std::cout << estimatedAlignment->estimate().cast<float>() << std::endl;*/
    pCurrentMap->SetGlobalVIOAlignment(estimatedAlignment->estimate().cast<float>());
    //delete VAlignment;
}

void Optimizer::AlignmentRefinement(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges, bool alignmentRotationFixed, bool bLarge, bool bRecInit)
{
    Map* pCurrentMap = pKF->GetMap();
    ...
    int vertexAlignmentId = maxKFid + 2;
    VAlignment->setId(vertexAlignmentId);
    VAlignment->setFixed(alignmentRotationFixed);
    optimizer.addVertex(VAlignment);
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(kfRelatedToGpsOrigin && kfRelatedToGpsOrigin->HasGlobalPositionMeas() && kfRelatedToGpsOrigin->mGlobalMeasCalib.mbIsSet && pKFi->HasGlobalPositionMeas())
        {            
            const GlobalPosition::GlobalPosition* meas = pKFi->GetGlobalPositionMeas()[0];
            const GlobalPosition::GlobalPosition* originMeas = kfRelatedToGpsOrigin->GetGlobalPositionMeas()[0];
            Sophus::SE3f Twb0 = kfRelatedToGpsOrigin->GetPoseInverse() * kfRelatedToGpsOrigin->GetPoseRelativeToKF(originMeas->id).inverse() * kfRelatedToGpsOrigin->mImuCalib.mTcb;
            Eigen::Vector3f tbg = kfRelatedToGpsOrigin->mGlobalMeasCalib.tbg;
            ...
            Eigen::Vector3d relativeMeasurement = meas->getRelativeFromOrigin(*originMeas);
            edge->setMeasurement(relativeMeasurement);
            edge->setInformation(meas->getCovariance().inverse().cast<double>());
            optimizer.addEdge(edge);
            edgesMeasCount++;
        }
    }
