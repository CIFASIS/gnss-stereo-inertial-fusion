...
namespace ORB_SLAM3 {
    ...
    Settings::Settings(const std::string &configFile, const int& sensor, const bool useGlobalMeas) :
    bNeedToUndistort_(false), bNeedToRectify_(false), bNeedToResize1_(false), bNeedToResize2_(false), bUseGlobalMeas_(useGlobalMeas) {
        sensor_ = sensor;
        ...
        if(bUseGlobalMeas_)
        {
            readGlobalMeasurements(fSettings);
        }
    ...
    void Settings::readGlobalMeasurements(cv::FileStorage &fSettings) 
    {
        bool found;
        cv::Mat cvtbg = readParameter<cv::Mat>(fSettings,"System.t_b_g",found);
        tbg_ = Converter::toVector3f(cvtbg);
    }
