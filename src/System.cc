    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    cout << endl <<
    "GNSS-SI Copyright (C) 2023 Javier Cremona, Ernesto Kofman and Taihú Pire, CIFASIS (CONICET-UNR)." << endl <<
    "GNSS-SI Copyright (C) 2023 Javier Civera, University of Zaragoza." << endl <<
    ...
    mbUseGlobalMeas_ = false;
    cv::FileNode globalMeasNode = fsSettings["System.UseGlobalMeas"];
    if(!globalMeasNode.empty())
    {
        mbUseGlobalMeas_ = static_cast<int>(globalMeasNode) != 0;
    }

    cout << "Use Global Measurements: " << mbUseGlobalMeas_ << endl;

    cv::FileNode node = fsSettings["File.version"];
    if(!node.empty() && node.isString() && node.string() == "1.0"){
        settings_ = new Settings(strSettingsFile,mSensor,mbUseGlobalMeas_);
    ...
    cout << "Loop Closing: " << activeLC << std::endl;
...
bool System::forceSystemShutdown(bool localizationMode)
{
    if(localizationMode)
    ...
    SaveTrajectoryEuRoC("CameraTrajectoryGPSOpt.txt");
    //mpSystem->SaveTrajectoryCustom("CameraTrajectory.txt");
    SaveKeyFrameTrajectoryEuRoC("KeyFrameTrajectory.txt");
    //SaveMapPointsFromCurrentMap();
    return true;
}
...
void System::SaveTrajectoryCustom(const string &filename)
{
...
void System::SaveKeyFrameTrajectoryEuRoC(const string &filename)
{
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
       ...
       // pKF->SetPose(pKF->GetPose()*Two);
        ...
        if(pKF->HasGlobalPositionMeas()){
            keyframeWithGPSMeas++;
        }
        //std::cout << pKF->mTimeStamp << "," << pKF->getGpsMeas().timestamp << std::endl;
    }
    f.close();
    std::cout << keyframeWithGPSMeas << " of " << vpKFs.size() << " keyframes have a GPS Measurement." << std::endl;
}
...
{
    cout << endl << "Saving map points..." << endl;
    vector<Map*> vpMaps = mpAtlas->GetAllMaps();
    ofstream f;
    f.open("map_points.txt", ios_base::app);
    f << fixed;
    ...
    for(Map* pMap :vpMaps) 
    {
        ...
        for(MapPoint* p: points)
        {
            if(!p->isBad())
            {
                Eigen::Vector3f coord = p->GetWorldPos();
...
void System::SaveMapPointsFromCurrentMap()
{
    cout << endl << "Saving map points from current map..." << endl;

    Map* pActiveMap = mpAtlas->GetCurrentMap();
    if(!pActiveMap)
    {
        cout << endl << "ERROR: No active map" << endl;
        return;
    }
    ...
    vector<MapPoint*> points = pActiveMap->GetAllMapPoints();
    for(MapPoint* p: points)
    {
        if(!p->isBad())
        {
