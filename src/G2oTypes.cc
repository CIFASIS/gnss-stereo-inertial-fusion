...
/*void EdgeGlobalMeasurement::linearizeOplus()
{
    const VertexPose* VP = static_cast<const VertexPose*>(_vertices[0]);
    const VertexRotation* VR = static_cast<const VertexRotation*>(_vertices[1]);
    _jacobianOplusXi.block<3,3>(0,0) = VR->estimate() * (VP->estimate().Rwb * Sophus::SO3d::hat(tbg));
    _jacobianOplusXi.block<3,3>(0,3) = -VR->estimate();

    Eigen::Vector3d estimated = (VP->estimate().Rwb * tbg + VP->estimate().twb) - (Twb0.rotationMatrix() * tbg + Twb0.translation());
    _jacobianOplusXj.block<3,3>(0,0) = VR->estimate() * Sophus::SO3d::hat(estimated); 
}*/
