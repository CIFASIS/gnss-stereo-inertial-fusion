...
KeyFrame::KeyFrame():
        ...
        NLeft(0),NRight(0), mnNumberOfOpt(0), mbHasVelocity(false), hasGlobalPositionMeasurement(false), globalPositionMeas(NULL)
{
...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    mImuCalib(F.mImuCalib), mGlobalMeasCalib(F.mGlobalMeasCalib), mvpMapPoints(F.mvpMapPoints), mpKeyFrameDB(pKFDB),
    ...
    mvKeysRight(F.mvKeysRight), NLeft(F.Nleft), NRight(F.Nright), mTrl(F.GetRelativePoseTrl()), mnNumberOfOpt(0), mbHasVelocity(false), hasGlobalPositionMeasurement(false), globalPositionMeas(NULL)
{
    mnId=nNextId++;
...
void KeyFrame::SetErase()
{
    ...
    if(mbToBeErased)
    {
        std::cout << "set erase: " << mnId << std::endl;
        SetBadFlag();
    }
}
...
{
    {
        ...
        else if (mpMap->HasKFRelatedToGlobalOrigin() && mnId == mpMap->GetKFRelatedToGlobalOrigin()->mnId)
        {
            return;
        }        
        else if(mbNotErase)
...
bool KeyFrame::HasGlobalPositionMeas()
{
    unique_lock<mutex> lock(mMutexGpsMeas);
    return hasGlobalPositionMeasurement;
}

/*void KeyFrame::SetGlobalPositionMeas(const GlobalPosition::GlobalPosition* meas)
{
    unique_lock<mutex> lock(mMutexGpsMeas);
    hasGlobalPositionMeasurement = true;
    //relativeToOriginMeas = relativeToOriginMeas_;
    globalPositionMeas = meas;
}*/

void KeyFrame::AddGlobalPositionMeas(const GlobalPosition::GlobalPosition* meas)
{
    unique_lock<mutex> lock(mMutexGpsMeas);
...
{
    unique_lock<mutex> lock(mMutexGpsMeas);
    if(frameRelativePoses.empty())
        return;

    map<double, Sophus::SE3f>::iterator framePoseIt = frameRelativePoses.begin();
    ...
    for(auto globalMeas : globalPositionMeasBeforeProp)
    {
        ...
        if(framePoseIt == frameRelativePoses.begin())
        {
            ...
            Sophus::SE3f TRelPrev = Sophus::SE3f();
            Sophus::SE3f TRelNext = nextFrame.second;
            double tsPrev = mTimeStamp;
            double tsNext = nextFrame.first;
            //std::cout << tsPrev << " " << globalMeas->timestamp << " " << tsNext << std::endl;
            double ratio = (tsNext - globalMeas->timestamp) / (tsNext - tsPrev);
            Sophus::SE3f Tcir = Sophus::interpolate(TRelPrev, TRelNext, ratio);
            globalPositionMeas.push_back(globalMeas);
            posesRelativeToKF[globalMeas->id] = Tcir;
            hasGlobalPositionMeasurement = true;
            pushed++;*/
        ...
        if(framePoseIt == frameRelativePoses.end())
        {
            if(mNextKF)
            {
                ...
                Sophus::SE3f TRelNext = mNextKF->GetPose() * GetPoseInverse();
                double tsPrev = prevFrame.first;
                double tsNext = mNextKF->mTimeStamp;
                double ratio = (tsNext - globalMeas->timestamp) / (tsNext - tsPrev);
                Sophus::SE3f Tcir = Sophus::interpolate(TRelPrev, TRelNext, ratio);
                globalPositionMeas.push_back(globalMeas);
                posesRelativeToKF[globalMeas->id] = Tcir;
        ...
        else
        {
            ...
            Sophus::SE3f TRelPrev = prevFrame.second;
            Sophus::SE3f TRelNext = currFrame.second;
            double tsPrev = prevFrame.first;
            double tsNext = currFrame.first;
            double ratio = (tsNext - globalMeas->timestamp) / (tsNext - tsPrev);
            Sophus::SE3f Tcir = Sophus::interpolate(TRelPrev, TRelNext, ratio);
            globalPositionMeas.push_back(globalMeas);
            posesRelativeToKF[globalMeas->id] = Tcir;
...
Sophus::SE3f KeyFrame::GetPoseRelativeToKF(double globalMeasId)
{
    unique_lock<mutex> lock(mMutexGpsMeas);
    return posesRelativeToKF.at(globalMeasId);
}

std::vector<const GlobalPosition::GlobalPosition*> KeyFrame::GetGlobalPositionMeas()
{
    unique_lock<mutex> lock(mMutexGpsMeas);
...
{
    unique_lock<mutex> lock(mMutexGpsMeas);
    frameRelativePoses[timestamp] = frameRelativePose;
}

/*Eigen::Vector3f KeyFrame::GetRelativeToOrigin()
{
    unique_lock<mutex> lock(mMutexGpsMeas);
    return relativeToOriginMeas;
}*/
