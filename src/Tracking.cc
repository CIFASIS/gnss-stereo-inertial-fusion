...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Atlas *pAtlas, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor, Settings* settings, const string &_nameSeq):
    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr), mpLastKeyFrame(static_cast<KeyFrame*>(NULL)), setGlobalPosOrigin(true), 
    globalFrameAlignmentState(NO_MEAS_YET)
{
    ...
    if(settings){
        ...
        bool b_parse_global_meas = true;
        if(mpSystem->UseGlobalMeas())
        {
            b_parse_global_meas = ParseGlobalMeasParamFile(fSettings);
            if(!b_parse_global_meas)
            {
                std::cout << "*Error with the Global Meas. parameters in the config file*" << std::endl;
            }
        }


        if(!b_parse_cam || !b_parse_orb || !b_parse_imu || !b_parse_global_meas)
        {
...
void Tracking::newParameterLoader(Settings *settings) {
    ...
    if(mpSystem->UseGlobalMeas())
        mpGlobalMeasCalib = GlobalPosition::Calib(settings->tbg());
...
bool Tracking::ParseGlobalMeasParamFile(cv::FileStorage &fSettings)
{
    bool b_miss_params = false;

    cv::Mat cvtbg;
    cv::FileNode node = fSettings["System.t_b_g"];
    if(!node.empty())
    {
        cvtbg = node.mat();
        if(cvtbg.rows != 3 || cvtbg.cols != 1)
        {
            std::cerr << "*tbg matrix have to be a 3D vector*" << std::endl;
            b_miss_params = true;
        }
    }
    else
    {
        std::cerr << "*tbg vector doesn't exist*" << std::endl;
        b_miss_params = true;
    }
    cout << endl;
    cout << "Global Sensor to Body traslation (tbg): " << endl << cvtbg << endl;
    Eigen::Vector3f tbg = Converter::toVector3f(cvtbg);
    //Eigen::Matrix<float,3,1,Eigen::RowMajor> eigtbg(cvtbg.ptr<float>(0));
    //Eigen::Vector3f tbg(eigtbg);
    if(b_miss_params)
    {
        return false;
    }
    mpGlobalMeasCalib = GlobalPosition::Calib(tbg);     
    return true;
}
...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename)
{
    ...
    if (mSensor == System::RGBD)
        mCurrentFrame = Frame(mImGray,imDepth,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,mpGlobalMeasCalib);
    else if(mSensor == System::IMU_RGBD)
        mCurrentFrame = Frame(mImGray,imDepth,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,mpGlobalMeasCalib,&mLastFrame,*mpImuCalib);
...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename)
{
    ...
    if (mSensor == System::MONOCULAR)
    {
        if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET ||(lastID - initID) < mMaxFrames)
            mCurrentFrame = Frame(mImGray,timestamp,mpIniORBextractor,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth,mpGlobalMeasCalib);
        else
            mCurrentFrame = Frame(mImGray,timestamp,mpORBextractorLeft,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth,mpGlobalMeasCalib);
    }
    else if(mSensor == System::IMU_MONOCULAR)
    {
        if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET)
        {
            mCurrentFrame = Frame(mImGray,timestamp,mpIniORBextractor,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth,mpGlobalMeasCalib,&mLastFrame,*mpImuCalib);
        }
        else
            mCurrentFrame = Frame(mImGray,timestamp,mpORBextractorLeft,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth,mpGlobalMeasCalib,&mLastFrame,*mpImuCalib);
    }
...
void Tracking::GrabGpsData(const GlobalPosition::GlobalPosition* gpsMeasurement)
{
...
list<const GlobalPosition::GlobalPosition *> Tracking::GetDataToAlign()
{
    unique_lock<mutex> lock(mMutexGlobalFrameAlignment);
    return dataToAlign;
}

void Tracking::SetGlobalFrameAlignmentState(GlobalFrameAlignmentState state)
{
    unique_lock<mutex> lock(mMutexGlobalFrameAlignment);
    globalFrameAlignmentState = state;
}

Tracking::GlobalFrameAlignmentState Tracking::GetGlobalFrameAlignmentState()
{
    unique_lock<mutex> lock(mMutexGlobalFrameAlignment);
    return globalFrameAlignmentState;
}

Eigen::Matrix3f AlignGlobalFrame(Eigen::Vector3f globalTraslationVector, Eigen::Vector3f estimatedTraslationVector)
{
    float yaw = acosf(globalTraslationVector.dot(estimatedTraslationVector) / (globalTraslationVector.norm() * estimatedTraslationVector.norm()));
...
void Tracking::CreateNewKeyFrame()
{
    ...
    if(setGlobalPosOrigin && mpReferenceKF->mPrevKF && mpReferenceKF->mPrevKF->HasGlobalPositionMeas())
    {
        ...
        //globalPositionOrigin = mpReferenceKF->GetGlobalPositionMeas()[0];
        mpAtlas->GetCurrentMap()->SetKFRelatedToGlobalOrigin(mpReferenceKF->mPrevKF); 
        std::cout << "SET FIRST GPS MEAS" << std::endl;
        SetGlobalFrameAlignmentState(FIRST_GLOBAL_MEAS_SET);
    }
...
void Tracking::ResetActiveMap(bool bLocMap)
{
    ...
    unique_lock<mutex> lock(mMutexGpsQueue);
    //mlQueueGpsData.clear();
    setGlobalPosOrigin = true;
    SetGlobalFrameAlignmentState(NO_MEAS_YET);
    dataToAlign.clear();
    globalPositionOrigin = static_cast<GlobalPosition::GlobalPosition*>(NULL);
    Verbose::PrintMess("   End reseting! ", Verbose::VERBOSITY_NORMAL);
}
...
void Tracking::OptimizeGlobalMeasurement()
{
    //Optimizer::OptimizeGraphGlobalMeas(mpAtlas->GetCurrentMap());
