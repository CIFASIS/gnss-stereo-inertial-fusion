...
Eigen::Matrix4f UmeyamaAlignment(std::list<Eigen::Vector3d> trajSrc, std::list<Eigen::Vector3d> trajDst)
{
    ...
    return Eigen::umeyama(M1,M2,false).cast<float>();
}
...
{    
    ...
    list<pair<double,Eigen::Vector3f>>::iterator framePosesDataIt = framePosesData.begin();
    std::list<const GlobalPosition::GlobalPosition*>::iterator gpsDataIt=gpsData.begin();

    std::list<Eigen::Vector3d> trajEstimated;
    std::list<Eigen::Vector3d> trajGlobalMeas;
    int iters = 0;
    while(framePosesDataIt != framePosesData.end() && gpsDataIt != gpsData.end())
    {
        iters++;
        ...
        if(framePosesDataIt->first - (*gpsDataIt)->timestamp > 0)
        {
            gpsDataIt++;
        }
        else
