...
void MapDrawer::DrawKeyFrames(const bool bDrawKF, const bool bDrawGraph, const bool bDrawInertialGraph, const bool bDrawOptLba)
{
    ...
    if(bDrawKF)
    {
        ...
        {
            KeyFrame* pKF = vpKFs[i];
            if(pActiveMap->HasKFRelatedToGlobalOrigin() && pKF->HasGlobalPositionMeas())
            {
                KeyFrame* originKF = pActiveMap->GetKFRelatedToGlobalOrigin();
                
                for(const GlobalPosition::GlobalPosition* meas : pKF->GetGlobalPositionMeas())
                {
                    auto originMeas = originKF->GetGlobalPositionMeas()[0];
                    Eigen::Vector3d relativeMeasurement = meas->getRelativeFromOrigin(*originMeas);
                    glColor3f(1.0,0.0,0.5);
                    glVertex3f(relativeMeasurement[0],relativeMeasurement[1],relativeMeasurement[2]);
                    Eigen::Vector3f estimated = Sophus::SE3f(pActiveMap->GetGlobalVIOAlignment(), Eigen::Vector3f::Zero()) * pKF->GetPoseInverse() * originKF->mGlobalMeasCalib.tbg;
                    glColor3f(0.0,0.0,1.0);
                    glVertex3f(estimated[0],estimated[1],estimated[2]);
                }                            
            }
        }
        glEnd();*/
