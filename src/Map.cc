...
firstGlobalPosAvailable(false), kfRelatedToGlobalOriginAvailable(false), kfRelatedToGlobalOrigin(static_cast<KeyFrame*>(NULL)), globalVIOAlignment(Eigen::Matrix3f::Identity())
{
    mnId=nNextId++;
...
void Map::SetFirstGlobalPos(Eigen::Vector3f globalPos)
{
    unique_lock<mutex> lock(mMutexMap);
    firstGlobalPosAvailable = true;
    firstGlobalPos = globalPos;
}

Eigen::Vector3f Map::GetFirstGlobalPos()
{
    unique_lock<mutex> lock(mMutexMap);
    return firstGlobalPos;
}

bool Map::HasFirstGlobalPos()
{
    unique_lock<mutex> lock(mMutexMap);
    return firstGlobalPosAvailable;
}

void Map::SetKFRelatedToGlobalOrigin(KeyFrame* kf)
{
    unique_lock<mutex> lock(mMutexMap);
    kfRelatedToGlobalOriginAvailable = true;
    kfRelatedToGlobalOrigin = kf;
}

void Map::ClearKFRelatedToGlobalOrigin()
{
    unique_lock<mutex> lock(mMutexMap);
    kfRelatedToGlobalOriginAvailable = false;
    kfRelatedToGlobalOrigin = static_cast<KeyFrame*>(NULL);;
}

KeyFrame* Map::GetKFRelatedToGlobalOrigin()
{
    unique_lock<mutex> lock(mMutexMap);
    return kfRelatedToGlobalOrigin;
}

void Map::SetGlobalVIOAlignment(Eigen::Matrix3f rotationAlignment)
{
    unique_lock<mutex> lock(mMutexMap);
    globalVIOAlignment = rotationAlignment;
}

Eigen::Matrix3f Map::GetGlobalVIOAlignment()
{
    unique_lock<mutex> lock(mMutexMap);
    return globalVIOAlignment;
}

bool Map::HasKFRelatedToGlobalOrigin()
{
    unique_lock<mutex> lock(mMutexMap);
    return kfRelatedToGlobalOriginAvailable;
}
