...
public:
    ...
    bool ParseGlobalMeasParamFile(cv::FileStorage &fSettings);
    ...
    void GrabGpsData(const GlobalPosition::GlobalPosition* gpsMeasurement);
    ...
    void OptimizeGlobalMeasurement();
...
public:
    ...
    void SetGlobalFrameAlignmentState(GlobalFrameAlignmentState state);

    GlobalFrameAlignmentState GetGlobalFrameAlignmentState();

    list<const GlobalPosition::GlobalPosition *> GetDataToAlign();
...
protected:
    ...
    // GPS measurements
    //std::list<const GlobalPosition::GlobalPosition*> mlQueueGpsData;
    std::list<const GlobalPosition::GlobalPosition*> dataToAlign;
    const GlobalPosition::GlobalPosition* globalPositionOrigin;
    bool setGlobalPosOrigin;
    // It allows to convert from lat/lon to local cartesian coordinates
    // GeographicLib::LocalCartesian gpsLocalCartesian;        

    // Vector of IMU measurements from previous to current frame (to be filled by PreintegrateIMU)
    std::vector<IMU::Point> mvImuFromLastFrame;
    std::mutex mMutexImuQueue;
    std::mutex mMutexGpsQueue;

    std::mutex mMutexGlobalFrameAlignment;

    // Imu calibration parameters
    IMU::Calib *mpImuCalib;

    GlobalPosition::Calib mpGlobalMeasCalib;
    ...
    GlobalFrameAlignmentState globalFrameAlignmentState;
