...
class VertexRotation : public g2o::BaseVertex<3,Eigen::Matrix3d>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    VertexRotation(){}
    VertexRotation(Eigen::Matrix3f _Rgw){
        setEstimate(_Rgw.cast<double>());
    }    
    ...
    virtual void oplusImpl(const double* update_){
        _estimate = _estimate * ExpSO3(update_[0], update_[1], update_[2]);
        updateCache();
    }
};
...
class VertexGlobalPosition : public g2o::BaseVertex<3, Eigen::Vector3d> {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
...
//class EdgeGlobalMeasurement : public g2o::BaseUnaryEdge<3,Eigen::Vector3d,VertexPose4DoF>
class EdgeGlobalMeasurement : public g2o::BaseBinaryEdge<3,Eigen::Vector3d,VertexPose,VertexRotation>
{
/**
 * If you define a structure having members of fixed-size vectorizable Eigen types,
 * you must ensure that calling operator new on it allocates properly aligned buffers. 
 * You have to overload its operator new so that it generates properly aligned pointers 
 * (e.g., 32-bytes-aligned for Vector4d and AVX). Fortunately, Eigen provides you with 
 * a macro EIGEN_MAKE_ALIGNED_OPERATOR_NEW that does that for you.
 * A Eigen::Vector4d consists of 4 doubles, which is 256 bits. This is exactly the size 
 * of an AVX register, which makes it possible to use AVX for all sorts of operations on this vector.
 * But AVX instructions (at least the ones that Eigen uses, which are the fast ones) require 256-bit alignment.
 * Otherwise you get a segmentation fault. Normally, you don't have to worry about anything, 
 * Eigen handles alignment of operator new for you except in one case. When you have a "class Foo"
 * like this, and you dynamically allocate a new Foo, then, since Foo doesn't have aligned operator new,
 * the returned pointer foo is not necessarily 256-bit aligned. The solution is 
 * to let class Foo have an aligned operator new, as we show here.
 * 
 */
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    
    EdgeGlobalMeasurement(Sophus::SE3d Twb0_, Sophus::SE3d Tcir_, Eigen::Vector3d tbg_):Twb0(Twb0_),Tcir(Tcir_),tbg(tbg_){}
    virtual bool read(std::istream& is){return false;}
    virtual bool write(std::ostream& os) const{return false;}
    //virtual void linearizeOplus();
    void computeError(){        
