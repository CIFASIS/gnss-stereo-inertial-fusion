...
public:
    ...
    bool HasGlobalPositionMeas();
    void AddGlobalPositionMeas(const GlobalPosition::GlobalPosition* meas);
    void ProcessGlobalPositionMeas();
    std::vector<const GlobalPosition::GlobalPosition*> GetGlobalPositionMeas();

    void AddFrameRelativePose(double timestamp, Sophus::SE3f frameRelativePose);
    Sophus::SE3f GetPoseRelativeToKF(double globalMeasId);
    //Eigen::Vector3f GetRelativeToOrigin();
...
public:
    ...
    // Global sensor calibration
    GlobalPosition::Calib mGlobalMeasCalib;    
...
protected:
    ...
    std::mutex mMutexMap;
    std::mutex mMutexGpsMeas;

    bool hasGlobalPositionMeasurement;
    std::vector<const GlobalPosition::GlobalPosition*> globalPositionMeas;
    std::vector<const GlobalPosition::GlobalPosition*> globalPositionMeasBeforeProp;
    //Eigen::Vector3f relativeToOriginMeas;

    std::map<double, Sophus::SE3f> frameRelativePoses;
    std::map<double, Sophus::SE3f> posesRelativeToKF;
public:
